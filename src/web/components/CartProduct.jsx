...
const CartProduct = (props) => {
  const { product } = props;
  const { actions: { changeValuesProductFromCart, deleteProductFromCart } } = useAppContext(); 
        ...
        <div className={styles.cartProductSelectQuantity}>
          {product.quantity < 10 ? (
            <div className={styles.cartProductContentSelect}>
              <span>Qty:</span>
              <span className={styles.cartProductQuantityValues}>{product.quantity}</span>
              <ChevronDownIcon />
              <select
            ...
            <div className={styles.cartProductInputContent}>
              <input type="number" defaultValue={product.quantity} className={styles.cartProductInputValues} onChange={handleInputChange} />
